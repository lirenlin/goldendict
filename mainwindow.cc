  ...
  newReleaseCheckTimer( this )
{
  ...
  QShortcut *shortcut2 = new QShortcut(QKeySequence("Ctrl+B"), this);
  connect(shortcut2, SIGNAL(activated()), wordbook, SLOT(activeWordBook()));
  connect(shortcut1, SIGNAL(activated()), ui.searchPane, SLOT(raise()));
  //wordbook->installEventFilter( this );
...
void MainWindow::handleEsc()
{
    ...
    if ( view && view->closeSearch() )
        return;
    /*
    if(wordbook && wordbook->closeSearch())
        return;
        */
...
{
    /// lirenlin, check if the search panel is on top of wordbook
    bool translateLineFocus = false;
    bool wordBookFocus = false;
    ...
    if(!tabList.isEmpty()){
        foreach(QTabBar * tabBar, tabList)
        {
            ...
            //qDebug() << currentTab;
            if(currentTab == ui.searchPane->windowTitle())
            {
                translateLineFocus = true;
                break;
            }
            if(currentTab == wordbook->windowTitle())
            {
                wordBookFocus = true;
                break;
            }
