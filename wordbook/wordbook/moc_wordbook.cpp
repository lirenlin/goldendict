/****************************************************************************
** Meta object code from reading C++ file 'wordbook.h'
**
** Created: Sun Nov 4 11:39:19 2012
**      by: The Qt Meta Object Compiler version 63 (Qt 4.8.1)
**
** WARNING! All changes made in this file will be lost!
*****************************************************************************/

#include "wordbook.h"
#if !defined(Q_MOC_OUTPUT_REVISION)
#error "The header file 'wordbook.h' doesn't include <QObject>."
#elif Q_MOC_OUTPUT_REVISION != 63
#error "This file was generated using the moc from 4.8.1. It"
#error "cannot be used with the include files from this version of Qt."
#error "(The moc has changed too much.)"
#endif

QT_BEGIN_MOC_NAMESPACE
static const uint qt_meta_data_WordBook[] = {

 // content:
       6,       // revision
       0,       // classname
       0,    0, // classinfo
       3,   14, // methods
       0,    0, // properties
       0,    0, // enums/sets
       0,    0, // constructors
       0,       // flags
       0,       // signalCount

 // slots: signature, parameters, type, tag, flags
      10,    9,    9,    9, 0x08,
      21,    9,    9,    9, 0x08,
      32,    9,    9,    9, 0x08,

       0        // eod
};

static const char qt_meta_stringdata_WordBook[] = {
    "WordBook\0\0importDB()\0exportDB()\0"
    "showSearchResult()\0"
};

void WordBook::qt_static_metacall(QObject *_o, QMetaObject::Call _c, int _id, void **_a)
{
    if (_c == QMetaObject::InvokeMetaMethod) {
        Q_ASSERT(staticMetaObject.cast(_o));
        WordBook *_t = static_cast<WordBook *>(_o);
        switch (_id) {
        case 0: _t->importDB(); break;
        case 1: _t->exportDB(); break;
        case 2: _t->showSearchResult(); break;
        default: ;
        }
    }
    Q_UNUSED(_a);
}

const QMetaObjectExtraData WordBook::staticMetaObjectExtraData = {
    0,  qt_static_metacall 
};

const QMetaObject WordBook::staticMetaObject = {
    { &QMainWindow::staticMetaObject, qt_meta_stringdata_WordBook,
      qt_meta_data_WordBook, &staticMetaObjectExtraData }
};

#ifdef Q_NO_DATA_RELOCATION
const QMetaObject &WordBook::getStaticMetaObject() { return staticMetaObject; }
#endif //Q_NO_DATA_RELOCATION

const QMetaObject *WordBook::metaObject() const
{
    return QObject::d_ptr->metaObject ? QObject::d_ptr->metaObject : &staticMetaObject;
}

void *WordBook::qt_metacast(const char *_clname)
{
    if (!_clname) return 0;
    if (!strcmp(_clname, qt_meta_stringdata_WordBook))
        return static_cast<void*>(const_cast< WordBook*>(this));
    return QMainWindow::qt_metacast(_clname);
}

int WordBook::qt_metacall(QMetaObject::Call _c, int _id, void **_a)
{
    _id = QMainWindow::qt_metacall(_c, _id, _a);
    if (_id < 0)
        return _id;
    if (_c == QMetaObject::InvokeMetaMethod) {
        if (_id < 3)
            qt_static_metacall(this, _c, _id, _a);
        _id -= 3;
    }
    return _id;
}
QT_END_MOC_NAMESPACE
