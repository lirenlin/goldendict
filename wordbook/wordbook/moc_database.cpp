/****************************************************************************
** Meta object code from reading C++ file 'database.h'
**
** Created: Sun Nov 4 11:39:20 2012
**      by: The Qt Meta Object Compiler version 63 (Qt 4.8.1)
**
** WARNING! All changes made in this file will be lost!
*****************************************************************************/

#include "database.h"
#if !defined(Q_MOC_OUTPUT_REVISION)
#error "The header file 'database.h' doesn't include <QObject>."
#elif Q_MOC_OUTPUT_REVISION != 63
#error "This file was generated using the moc from 4.8.1. It"
#error "cannot be used with the include files from this version of Qt."
#error "(The moc has changed too much.)"
#endif

QT_BEGIN_MOC_NAMESPACE
static const uint qt_meta_data_DB[] = {

 // content:
       6,       // revision
       0,       // classname
       0,    0, // classinfo
       5,   14, // methods
       0,    0, // properties
       0,    0, // enums/sets
       0,    0, // constructors
       0,       // flags
       1,       // signalCount

 // signals: signature, parameters, type, tag, flags
       4,    3,    3,    3, 0x05,

 // slots: signature, parameters, type, tag, flags
      40,   21,   16,    3, 0x0a,
      85,   80,   72,    3, 0x0a,
     103,   80,    3,    3, 0x0a,
     126,  122,    3,    3, 0x0a,

       0        // eod
};

static const char qt_meta_stringdata_DB[] = {
    "DB\0\0dbChanged()\0bool\0name,mean,dictName\0"
    "addRow(QString,QString,QString)\0QString\0"
    "name\0dictName(QString)\0removeRow(QString)\0"
    "row\0removeRow(int)\0"
};

void DB::qt_static_metacall(QObject *_o, QMetaObject::Call _c, int _id, void **_a)
{
    if (_c == QMetaObject::InvokeMetaMethod) {
        Q_ASSERT(staticMetaObject.cast(_o));
        DB *_t = static_cast<DB *>(_o);
        switch (_id) {
        case 0: _t->dbChanged(); break;
        case 1: { bool _r = _t->addRow((*reinterpret_cast< QString(*)>(_a[1])),(*reinterpret_cast< QString(*)>(_a[2])),(*reinterpret_cast< QString(*)>(_a[3])));
            if (_a[0]) *reinterpret_cast< bool*>(_a[0]) = _r; }  break;
        case 2: { QString _r = _t->dictName((*reinterpret_cast< QString(*)>(_a[1])));
            if (_a[0]) *reinterpret_cast< QString*>(_a[0]) = _r; }  break;
        case 3: _t->removeRow((*reinterpret_cast< QString(*)>(_a[1]))); break;
        case 4: _t->removeRow((*reinterpret_cast< int(*)>(_a[1]))); break;
        default: ;
        }
    }
}

const QMetaObjectExtraData DB::staticMetaObjectExtraData = {
    0,  qt_static_metacall 
};

const QMetaObject DB::staticMetaObject = {
    { &QObject::staticMetaObject, qt_meta_stringdata_DB,
      qt_meta_data_DB, &staticMetaObjectExtraData }
};

#ifdef Q_NO_DATA_RELOCATION
const QMetaObject &DB::getStaticMetaObject() { return staticMetaObject; }
#endif //Q_NO_DATA_RELOCATION

const QMetaObject *DB::metaObject() const
{
    return QObject::d_ptr->metaObject ? QObject::d_ptr->metaObject : &staticMetaObject;
}

void *DB::qt_metacast(const char *_clname)
{
    if (!_clname) return 0;
    if (!strcmp(_clname, qt_meta_stringdata_DB))
        return static_cast<void*>(const_cast< DB*>(this));
    return QObject::qt_metacast(_clname);
}

int DB::qt_metacall(QMetaObject::Call _c, int _id, void **_a)
{
    _id = QObject::qt_metacall(_c, _id, _a);
    if (_id < 0)
        return _id;
    if (_c == QMetaObject::InvokeMetaMethod) {
        if (_id < 5)
            qt_static_metacall(this, _c, _id, _a);
        _id -= 5;
    }
    return _id;
}

// SIGNAL 0
void DB::dbChanged()
{
    QMetaObject::activate(this, &staticMetaObject, 0, 0);
}
QT_END_MOC_NAMESPACE
